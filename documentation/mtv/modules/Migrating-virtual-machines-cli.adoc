...
<4> Specify VMs to migrate.

. Create a Migration object to run the Plan:
+
[source,terminal]
----
$ cat < EOF | oc apply -f -
...
The Migration creates a `VirtualMachineImport` object for each VM being migrated. You can monitor the progress of the migration by viewing the `VirtualMachineImport` pods in the `openshift-migration` namespace.
